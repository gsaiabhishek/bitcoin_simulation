...
def sendEmptyLoopTransaction(address):
    ...
    tx_hash = sort_contract.functions.runLoop(0).transact({'txType':"0x3", 'from':w3.eth.accounts[0], 'gas':2409638})
    # retVal = w3.eth.getTransactionReceipt(tx_hash)['logs'][0]['args']['_value']
...
if receipt3 is not None:
    ...
    print(receipt3)
    print(receipt3['logs'][0]['data'])
    print(type(receipt3['logs'][0]['data']))
